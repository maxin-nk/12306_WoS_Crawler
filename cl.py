...
def download_file(i, url1, browser, stastistic):
    time.sleep(2)
    k = 1
    for ith in range(10):
        try:
            browser.find_by_xpath("//app-records-list[@class='app-records-list']"
                                  "/app-record[" + str(i + 1) +"]/div[2]/div[1]/app-summary-title/h3/a").click()
            browser.execute_script(js)
            break
        except:
            if ith == 0:
                gundong(math.ceil(i/5))
            elif ith != 9:
                gundong(math.ceil(i / 5-k))
                k += 1
                time.sleep(0.5)
            else:
                return False, '第%d篇文献' % stastistic
...
if __name__ == '__main__':
    URL_L = []
    qikans = ['ENERGY & ENVIRONMENTAL SCIENCE', 'PROGRESS IN ENERGY AND COMBUSTION SCIENCE',
    ...
    for qikan in qikans:
        ...
        ys = int(ys.replace(',', ''))
        stastistic = 0
        YESHU = range(math.ceil(ys / 50))
        for j in YESHU:
            if j < math.floor(ys / 50):
                last = 50
            else:
                last = ys - math.floor(ys / 50) * 50
            xun = range(last)
            stastistic2 = 0
            while stastistic2 < last:
                try:
                    for i in xun:
                        url1 = browser.url  # 保存当前界面
                        panduan, url_l = download_file(i, url1, browser, stastistic)
                        # if i % 3 == 0:
                        #     gundong(math.ceil(i / 3))
                        if not panduan:
                            URL_L.append(url_l)    # 没有查到的
                            print(url_l)
                        stastistic += 1
                        stastistic2 += 1
                    browser.find_by_xpath('//*[@id="snNextPageBottom"]').fill(j + 1)
                    time.sleep(0.5)
                    browser.find_by_xpath('/html/body/app-wos/div/div/main/div').click()
                    time.sleep(0.5)
                    break
                except:
                    for ith in range(2):
                        try:
                            if ith == 0:
                                wait(1)
                                time.sleep(0.5)
                                browser.find_by_xpath('//*[@id="pendo-button-506b4382"]').click()
                                break
                            else:
                                browser.find_by_xpath('//*[@id="pendo-button-506b4382"]').click()
                        except:
                            print('没有第%d类弹窗'%(ith+1))
                    print('stastistic:', stastistic, qikan)
                    browser.reload()
                    time.sleep(0.5)
                    xun = range(stastistic2, last)
