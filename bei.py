...
# bj = random.sample(user_agent,1)[0]
bj = user_agent[20]
print(bj)
browser = Browser(driver_name='chrome',user_agent=bj, **executable_path)
                ...
                if browser.find_by_xpath('// *[@id = "FullRecordExportToEnwBtnover"]').first.visible and ii != 1:
                    ...
                    if browser.find_by_xpath('// *[@id = "FullRecordExportToEnwBtnover"]').first.visible:
                        if ii == 2:
                            ...
                            browser.quit()
                            time.sleep(0.5)
                            browser = Browser(driver_name='chrome', user_agent=user_agent[0], **executable_path)
...
def fanye(browser,each_page):
    # 根据当前browser进行翻页，不行就一个个点
    try:
...
def fanye2(browser, each_page):
    browser.find_by_id("snNextPageTop").fill(str(each_page))
    # 翻页
    time.sleep(0.5)
    e2 = browser.find_by_id('FullRTa-fullRecordtitle-0')
    try:
...
def save_data(data, title11, TITLE11, YJFX11, WBOS11, each_page):
    if data is None:
    ...
    if each_page % 1 == 0 and each_page != 0:
        data.to_excel(r'D:\data\citation' + str(each_page) + '.xls')
        data = None
        print('success,%d' % each_page)
    return data


def reset(each_page,qikan):
    time.sleep(1)
    browser = Browser(driver_name='chrome', user_agent=user_agent[0], **executable_path)
    jicao(browser,qikan)
    wait(browser)
